class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        # Sorting the car stack by position, then arrival time for tie-breaking
        car_stack.sort() 
    
        # Keep track of the current current fleet max time (fleets in front of the current car)
        fleet_max = 0
